...
def train(
    ...
    if model == 'lenet':
        ...
        hnet: nn.Module = ResnetHyper(
            hidden_dim=hidden_dim, **hn_config[resnet_size])
        ...
        if (epoch + 1) % eval_every == 0:
            ...
            if not no_val_eval:
                epoch_results = evaluate(
                    hypernet=hnet, targetnet=net, loader=val_loader, rays=test_rays, device=device)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--datapath', type=str,
                        default='data/multi_fashion_and_mnist.pickle', help='path to data')
    parser.add_argument('--n-epochs', type=int,
                        default=150, help='num. epochs')
    parser.add_argument('--ray-hidden', type=int,
                        default=100, help='lower range for ray')
    parser.add_argument('--alpha', type=float, default=.2,
                        help='alpha for dirichlet')
    parser.add_argument('--model', type=str, default='lenet',
                        choices=['lenet', 'resnet'], help='model name')
    parser.add_argument(
        '--resnet-size', type=str, default='11M', choices=['11M', '5M', '2M', '1M'],
        help='ResNet size key. Only used if model set to resnet'
    )
    parser.add_argument('--no-cuda', action='store_true',
                        default=False, help='train on gpu')
    ...
    parser.add_argument('--val-size', type=float,
                        default=.1, help='validation size')
    parser.add_argument('--no-val-eval', action='store_true',
                        default=False, help='evaluate on validation')
    parser.add_argument('--solver', type=str,
                        choices=['ls', 'epo'], default='epo', help='solver')
    parser.add_argument('--eval-every', type=int, default=10,
                        help='number of epochs between evaluations')
    parser.add_argument('--out-dir', type=str,
                        default='outputs', help='outputs dir')
